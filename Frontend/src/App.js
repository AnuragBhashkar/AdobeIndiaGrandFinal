...
function App() {
  ...
  // Set the token for all API requests if it exists
  if (userToken) {
  ...
  const handleLogin = (name, token) => {
    ...
    setUserName(name);
    setUserToken(token);
    setAuthToken(token); // Set token on login
    setIsModalOpen(false);
  };
...
// Wrap App in the ThemeContextProvider so all components can access the theme
const AppWrapper = () => (
  <ThemeContextProvider>
    <App />
  </ThemeContextProvider>
